    ...
    pub(crate) fn allocate_bars(
        ...
        while bar_id < VFIO_PCI_CONFIG_REGION_INDEX {
            ...
            if let Some(resources) = &resources {
                ...
                // And read back BAR value. The device will write zeros for bits it doesn't care
                // about
...
impl Debug for VfioPciDevice {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        f.debug_struct("VfioPciDevice").finish()
    }
}
...
impl VfioPciDevice {
    ...
    /// # Arguments
    ///
    /// * `vm` - The VM object. It is used to set the VFIO MMIO regions as user memory regions.
            ...
            match mem.get_host_address(guest_addr) {
                ...
                Err(e) => {
                    return Err(io::Error::new(
                        io::ErrorKind::Other,
                        format!(
                            "unable to retrieve user address for gpa 0x{gpa:x} from guest memory \
                             region: {e}"
                        ),
                    ));
                }
        ...
        self.container
            ...
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::Other,
                    format!(
                        "failed to map memory for VFIO container, iova 0x{iova:x}, gpa 0x{gpa:x}, \
                         size 0x{size:x}: {e:?}"
                    ),
                )
            })
    ...
    fn unmap(&self, iova: u64, size: u64) -> std::result::Result<(), io::Error> {
        self.container.vfio_dma_unmap(iova, size).map_err(|e| {
            io::Error::new(
                io::ErrorKind::Other,
                format!(
                    "failed to unmap memory for VFIO container, iova 0x{iova:x}, size 0x{size:x}: \
                     {e:?}"
                ),
            )
        })
