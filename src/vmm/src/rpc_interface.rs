...
pub enum VmmAction {
    ...
    /// Set the memory hotplug device using `MemoryHpConfig` as input. This action can only be called
    /// before the microVM has booted.
...
impl RuntimeApiController {
    ...
    pub fn handle_request(&mut self, request: VmmAction) -> Result<VmmData, VmmActionError> {
        ...
        match request {
            ...
            UpdateMemoryHp(memory_hp_update) => self.update_memory_hp(memory_hp_update),
    ...
    /// Updates the memory hotplug device requested size.
    fn update_memory_hp(
        &mut self,
        memory_hp_update: MemoryHpUpdateConfig,
    ) -> Result<VmmData, VmmActionError> {
        self.vmm
            .lock()
            .expect("Poisoned lock")
            .update_memory_hp_config(memory_hp_update.requested_size_mib)
            .map(|()| VmmData::Empty)
            .map_err(VmmActionError::InternalVmm)
    }
}
...
mod tests {
    ...
    fn test_preboot_disallowed() {
        ...
        check_unsupported(preboot_request(VmmAction::UpdateMemoryHp(
            MemoryHpUpdateConfig { requested_size_mib: 0 },
        )));
        #[cfg(target_arch = "x86_64")]
    ...
    fn test_runtime_disallowed() {
        ...
        check_unsupported(runtime_request(VmmAction::UpdateMemoryHp(
            MemoryHpUpdateConfig { requested_size_mib: 0 },
        )));
    }
}
