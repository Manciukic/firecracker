...
impl GuestMemoryExtension for GuestMemoryMmap {
    ...
    fn dump_dirty<T: WriteVolatile + std::io::Seek>(
        ...
        let write_result = dirty_bitmap.iter().try_for_each(|(base, kvm_bitmap)| {
            ...
            let region_offset = u64_to_usize(region.start_addr().checked_offset_from(base_addr).unwrap());
            ...
            for (i, v) in kvm_bitmap.iter().enumerate() {
                for j in 0..64 {
                    ...
                    let is_firecracker_page_dirty = firecracker_bitmap.dirty_at(region_offset + page_offset);
