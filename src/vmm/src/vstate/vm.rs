...
impl Vm {
    ...
    pub fn put_memory_region(&mut self, region: GuestRegionMmap, register: bool) -> Result<(), VmError> {
        if register {
    ...
    pub fn register_memory_region(&self, userspace_addr: u64, start_addr: GuestAddress, len: GuestUsize, track_dirty_pages: bool) -> Result<(), VmError> {
    ...
    pub fn reset_dirty_bitmap(&self) {
        self.common.kvm_memory_slots
            .lock()
            .unwrap()
            .iter()
            .for_each(|region| {
                let _ = self.fd().get_dirty_log(region.slot, u64_to_usize(region.memory_size));
            });
    }
