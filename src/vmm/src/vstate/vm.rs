...
use crate::vstate::memory::{
    Address, GuestMemory, GuestMemoryExtension, GuestMemoryMmap, GuestMemoryRegion, GuestMmapRegion, GuestRegionMmap
};
...
fn mincore_bitmap(region: &GuestMmapRegion) -> Result<Vec<u64>, VmError> {
    ...
    let mut mincore_bitmap = vec![0u8; region.len() / page_size];
    let mut bitmap = vec![0u64; (region.len() / page_size).div_ceil(64)];
