...
impl Debug for PciSegment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PciSegment").field("id", &self.id).finish()
    }
}
...
impl PciSegment {
    pub(crate) fn new(
        ...
        let pci_bus = Arc::new(Mutex::new(PciBus::new(pci_root, device_relocation)));
        ...
        info!(
            "Adding PCI segment: id={}, PCI MMIO config address: 0x{:x}, mem32 area \
             [0x{:x}-0x{:x}, mem64 area [0x{:x}-0x{:x}",
            segment.id,
    ...
    //     )?;
    //     let pci_config_io =
    // Arc::new(Mutex::new(BusDevice::PioPciBus(PciConfigIo::new(Arc::clone(&segment.pci_bus)))));
