...
use crate::devices::virtio::mem::response::{Response, ResponseCode, ResponseStateCode, ResponseType};
...
impl VirtioMem {
    ...
    fn handle_plug_request(&mut self, request: &Request) -> Result<(), VirtioMemError> {
        ...
        if end_block > self.total_blocks as u64 {
            return self.write_response(request, &Response {
    ...
    fn handle_unplug_request(&mut self, request: &Request) -> Result<(), VirtioMemError> {
        ...
        for block_idx in start_block..end_block {
            if self.is_block_plugged(block_idx as usize) {
                ...
                unsafe {
                    libc::madvise(addr as *mut libc::c_void, VIRTIO_MEM_BLOCK_SIZE, libc::MADV_DONTNEED);
                }
            }
        }
    ...
    fn handle_state_request(&mut self, request: &Request) -> Result<(), VirtioMemError> {
        ...
        if req.addr % self.config.block_size != 0 || req.nb_blocks == 0 || end_block > self.total_blocks as u64 {
            return self.write_response(request, &Response {
    ...
    pub fn update_requested_size(&mut self, requested_size: u64, vm: &Vm) -> Result<(), VirtioMemError> {
        ...
        if self.config.usable_region_size < requested_size {
            vm.register_memory_region(
                ...
                GuestAddress(self.config.addr).checked_add(self.config.usable_region_size).unwrap(),
        ...
        debug!("virtio-mem: Updated requested size to {} bytes", requested_size);
        self.irq_trigger
