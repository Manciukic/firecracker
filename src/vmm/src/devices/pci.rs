...
pub struct PciBus {
    ...
    devices: HashMap<u32, Arc<Mutex<BusDevice>>>,
    device_ids: Vec<bool>,
}

impl PciBus {
    pub fn new(pci_root: BusDevice) -> Self {
        let mut devices: HashMap<u32, Arc<Mutex<BusDevice>>> = HashMap::new();
    ...
    pub fn remove_by_device(&mut self, device: &Arc<Mutex<BusDevice>>) -> Result<()> {
...
impl Debug for PciConfigIo {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        f.debug_struct("PciConfigIo").finish()
    }
}
    ...
    pub fn config_space_write(&mut self, offset: u64, data: &[u8]) -> Option<Arc<Barrier>> {
        ...
        if let Some(d) = pci_bus.devices.get(&(device as u32)) {
            ...
            if let Some(params) = device
                .pci_device_mut()
                .unwrap()
                .detect_bar_reprogramming(register, data)
            {
...
impl Debug for PciConfigMmio {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        f.debug_struct("PciConfigMmio").finish()
    }
}
