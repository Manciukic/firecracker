...
impl VmResources {
    ...
    fn allocate_guest_memory_regions(&self, regions: Vec<(GuestAddress, usize)>) -> Result<Vec<GuestRegionMmap>, MemoryError> {
    ...
    pub fn allocate_guest_hotpluggable_memory(&self) -> Result<Vec<GuestRegionMmap>, MemoryError> {
        ...
        let regions = vec![(VIRTIO_MEM_GUEST_ADDRESS, mib_to_bytes(self.memory_hp.as_ref().unwrap().total_size_mib))];
        self.allocate_guest_memory_regions(regions)
    }
}
